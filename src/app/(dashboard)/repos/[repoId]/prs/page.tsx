...
// ─── MAIN PR INSIGHTS PAGE FOR SPECIFIC REPO ───
export default function RepoPRsPage() {
  const params = useParams();
  const repoId = (params?.repoId as string) || "";
  const [expandedPR, setExpandedPR] = useState<number | null>(null);

  // 1. Fetch main PR insights data (60s refetch interval)
  const { data: insightsData, isLoading: isLoadingInsights, error: insightsError } = useQuery<PRInsightsResponse>({
    queryKey: ["repo-pr-insights", repoId],
    queryFn: async () => {
  ...
  if (isLoadingInsights || isLoadingHistory) {
    return (
      <div className="space-y-6 max-w-5xl mx-auto px-6 py-6">
        {/* KPI Row skeletons */}
  ...
  if (insightsError) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6">
        <AlertCircle className="w-12 h-12 text-[#842029] mb-4" />
        <h3 className="text-lg font-bold text-[#1C1B1F]">Failed to load PR Insights</h3>
        <p className="text-sm text-[#49454F] mt-1">
          Make sure the repository is connected and has completed its onboarding sync.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#E4E1EC] pb-5">
        <div>
          <span className="text-xs font-bold text-[#6E56F2] uppercase tracking-wider">Repository Workspace</span>
          <h1 className="text-2xl md:text-3xl font-extrabold text-[#1C1B1F] tracking-tight mt-0.5">
            {insightsData?.repoFullName || repoId}
          </h1>
        </div>
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-[#2bee4b]" />
          <span className="text-xs font-semibold text-[#49454F]">PR Monitoring Active</span>
        </div>
      </div>
